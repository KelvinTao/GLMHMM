...
from .emitGenerate_X import _emit_generate
from .emitGenerate_X import _emit_generate_z0
...
class GLMHMMEstimator(BaseEstimator):
    ...
    def predict(self, X): 
        ...
        Returns
        -------
        y_pred : array-like, shape (These should be in the form of a numpy array with size (time) containing integer numbers from 0...N-1 (N: the number of possible outputs, i.e. song types) per sample in a list).
            The target values (Class labels in classification).
        y_lik_pred: likelyhood for each labels/symbols with size (N, time) per sample in a list, N: the number of possible outputs.
        ##
        z_pred : hidden state (time) per sample in a list.
        z_lik_pred: (number, time) number: the number of possible hidden states.
        """

        total_trials = len(X)  # How many different trials are we fitting?
        ##
        #total_trials=5
        res=[[] for i in range(total_trials)]
        y_pred=[[] for i in range(total_trials)]
        y_lik_pred=[[] for i in range(total_trials)]
        z_pred=[[] for i in range(total_trials)]
        z_lik_pred=[[] for i in range(total_trials)]
        ##
        for trial in range(0, total_trials):
            [y_pred[trial],y_lik_pred[trial],z_pred[trial],z_lik_pred[trial]]=_emit_generate(self.emit_w_, self.trans_w_, X[trial])

        return y_pred,y_lik_pred,z_pred,z_lik_pred
    

    def predict_one_segment(self, x, z0 = np.nan, z0_lik = np.nan):
        ##x is one segment
        [y_pred,y_lik_pred,z_pred,z_lik_pred]=_emit_generate_z0(self.emit_w_, self.trans_w_, x, z0, z0_lik)
