...
class GLMHMMEstimator(BaseEstimator):
    ...
    folds: int, defaults to 5.
        The number of folds, always 5 or 10.
    NO_test: int, defaults to 1
        The NO of test set with in folds, from 1 to folds.
    CV_random: int, defaults to 200
        The random numer seed used for permutation samples.
    ...
    def fit(self, X, y, y_analog):   
        ...
        # as well as the likelihood of seeing a transition from state to state       
        effective_ind = 0
        last_try = False
        ...
        for ind in range(0, self.max_iter):
                ...
                if self.cross_validate == True or self.CV_regularize == True:
                    ...
                    for this_lambda in CV_schedule:
                        ...
                        if self.symb_exists == True:
                            ...
                            # field...
                            [new_emit_w_temp, pgd_lik, pgd_prob1, pgd_prob2, pgd_prob3, pgd_prob4] = _fit_emission_filters(X, y, gamma, xi, self.emit_w_, self.get_params(), self.train_data_)
                        ...
                        if self.analog_flag == True:
                            ...
                            [test_emit_lik_temp, test_trans_lik_temp, test_analog_lik_temp, test_gamma_temp, test_xi_temp] = _HMMGLM_likelihoods(test_symb, new_emit_w[CV_ind], new_trans_w[CV_ind], test_stim, np.array([]), np.array([]), self.get_params())
                    ...
                    if self.CV_regularize == True:
                        ...
                        good_ind = np.argwhere(test_log_lik == min(test_log_lik))[0,0]##if multi min
                    ...
                    if self.cross_validate == True:
                        ...
                        #output[ind]['emitlik_CV_all'] =test_full_emit_lik
                else:
            ...
            output[ind]['emit_w'] = self.emit_w_
            output[ind]['trans_w'] = self.trans_w_ 
    ...
    def _initialize_parameters(self, X, random_state):       
        ...
        if self.cross_validate == False:
            ...
            np.random.seed(self.CV_random)
            rp_data = np.random.permutation(len(X))
            part=int(np.round(len(rp_data)/self.folds))
            if self.NO_test==1:
                self.test_data_=rp_data[0:part]
                self.train_data_=rp_data[part:]
            elif self.NO_test==self.folds:
                startI=part*(self.folds-1)
                self.test_data_=rp_data[startI:]
                self.train_data_=rp_data[0:startI]
            else:
                startI=part*(self.folds-1)
                endI=part*self.folds
                self.test_data_=rp_data[startI:endI]
                self.train_data_=np.hstack((rp_data[0:startI],rp_data[endI:]))
    ...
    def _m_step_emission(self, new_stim):
        ...
        for i in range(0, self.num_states):
            if self.num_steps == 1:
                ...
                outweights = _minimize_LBFGS(lambda x: _emit_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2]), order = 'F'), lr = 1, max_iter = 50, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = True)
            else:
                ...
                outweights = _minimize_LBFGS(lambda x: _emit_multistep_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2]), order = 'F'), lr = 1, max_iter = 50, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = True)
    ...
    def _m_step_transition(self, new_stim):
        ...
        for i in range(0, self.num_states):           
            if self.train_bins.size == 0:
                ...
                outweights = _minimize_LBFGS(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, :].T, (self.trans_w_.shape[1] * self.trans_w_.shape[2]), order = 'F'), lr = 1, max_iter = 50, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = True)
                ...
                outweights = _minimize_LBFGS(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, self.train_bins].T, (self.trans_w_.shape[1] * len(self.train_bins)), order = 'F'), lr = 1, max_iter = 50, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 25, history_size = 100, out = True)
