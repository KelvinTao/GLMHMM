...
def _fast_ASD_weighted_group(x, y, w, nk_grp, min_length, nx_circ = np.nan):
    ...
    ##modification!!!!
    nk_grp=np.expand_dims(np.array([nk_grp[0][0][0],nk_grp[1][0]]),1)
    ...
    rho_range = [rho_min[0][0], rho_max[0][0]]
    ...
    trho_range = np.sqrt(2 * np.pi) * np.array(rho_range) * np.array([min(length_range)[0], max(length_range)[0]])
    ...
    for i in range(0, n_grp):
        opt1['nx_circ'] = opts['nx_circ'][i,0] # pass in ju
        ...
        print(B_mats_temp.shape)
        B_mats_shape_0[i] = B_mats_temp.shape[0]
        B_mats_shape_1[i] = B_mats_temp.shape[1]
        
        n_w_vec[i] = len(w_vecs_per_grp)
        
    Bfft = np.zeros((int(np.sum(B_mats_shape_0, axis = 0)), int(np.sum(B_mats_shape_1, axis = 0))))
    w_vec = np.array([])
     
    for i in range(0, n_grp):
        Bfft[int(np.sum(B_mats_shape_0[0:i], axis = 0)):int(np.sum(B_mats_shape_0[0:i + 1], axis = 0)), int(np.sum(B_mats_shape_1[0:i], axis = 0)):int(np.sum(B_mats_shape_1[0:i + 1], axis = 0))] = B_mats[i] # Fourier basis matrices assembled into block diag
    ...
    B_grp = np.zeros((int(np.sum(n_w_vec, axis = 0)), n_grp))
    for i in range(0, n_grp):
        B_grp[int(np.sum(n_w_vec[0:i], axis = 0)):int(np.sum(n_w_vec[0:i + 1], axis = 0)), i] = np.ones((int(n_w_vec[i]), 1))
...
def _mkcov_ASD_factored(prs, nx, opts = np.nan):
    ...
    if max_freq < opts['nx_circ'] / 2:
        ...
        w_vec = np.concatenate((np.arange(0, n_cos), np.arange(-n_sin, 0))).T # vector of frequencies
    ...
    U, _ = _realfftbasis(nx, opts['nx_circ'], w_vec)
    U=U.T
...
def _mkcovdiag_ASD(length, rho, nx_circ, w_vec_sq):
    ...
    dc = [(1 / length - length * ww) * c_diag, (1 / rho) * np.ones(nw)] # dC / dl, dC / drho
    ...
    ddc_inv = [(2 / length ** 2 - ww + (length ** 2) * np.power(ww, 2)), (1 / rho) * (1 / length - length * ww), (2 / rho ** 2) * np.ones(nw)] / c_diag # d^2 C^-1 / dl^2, d^2 C^-1 / drho dl, d^2 C^-1 / drho^2
...
def _neglogev_ASD_spectral(prs, dd, ww_nrm, cond_thresh):
    ...
    ni = int(np.sum(ii, axis = 0)) # rank of covariance after pruning
...
def _neglogev_ASD_spectral_group(prs, dd, B_grp, ww_nrm, cond_thresh):
    ...
    ni = int(np.sum(ii, axis = 0)) # number of non-zero DFT coefs / rank of covariance after pruning
...
def _realfftbasis(nx, nn = np.nan, w_vec = np.nan):
    ...
    xc = np.arange(0, len(w_cos)).T # spatial pixel indices
    xs = np.arange(0, len(w_sin)).T # spatial pixel indices
    
    if w_sin.size != 0:
        #B = [np.cos((w_cos * 2 * np.pi / nn) * x.T), np.sin((w_sin * 2 * np.pi / nn) * x.T)] / np.sqrt(nn / 2)
        B = [np.cos((w_cos * 2 * np.pi / nn) * xc.T), np.sin((w_sin * 2 * np.pi / nn) * xs.T)] / np.sqrt(nn / 2)
    else:
        B = np.cos((w_cos * 2 * np.pi / nn) * x.T) / np.sqrt(nn / 2)
    
    # Make DC term into a unit vector
    izero = np.argwhere(w_vec == 0)[0][0] # index for DC term
    #print(B.shape)
