...
def _GLMHMM_analog_lik(analog_emit_w, X, y_analog, num_analog_emit):
    ...
    for analog_num in range(num_analog_params - 1, -1, -1):
        #residual_bag[analog_num] = np.zeros((num_states, int(num_analog_emit[analog_num])))
        residual_bag[analog_num] = np.zeros((num_states, int(len(y_analog)*y_analog[0].shape[1])))
        analog_residuals = []
        for trial in range(0, len(y_analog)):
            ...
            analog_residuals.append(np.zeros((y_analog[trial].shape[0], num_states, y_analog[trial].shape[1])))
    ...
    for trial in range(0, len(y_analog)):
        for analog_num in range(0, num_analog_params):
            ...
            good_emit = ~np.isnan(y_analog[trial][analog_num, :])
            ...
            analog_residuals[trial][analog_num, :, :] = np.tile(np.expand_dims(y_analog[trial][analog_num, good_emit], axis = 0), (num_states, 1)) - np.reshape(prediction, (num_states, T))
            residual_bag[analog_num][:, int(analog_ind[analog_num]):int(analog_ind[analog_num] + T)] = analog_residuals[trial][analog_num, :, :]

            #if np.sum(np.isnan(analog_residuals[trial]), axis = 0) > 0:
            if np.isnan(analog_residuals[trial]).any():
                print('Shit!')
    ...
    for trial in range(0, len(y_analog)):
        ...
        analog_lik.append(np.zeros((num_analog_params, num_states, y_analog[trial].shape[1])))
        for analog_num in range(0, num_analog_params):
            for ss in range(0, num_states):
