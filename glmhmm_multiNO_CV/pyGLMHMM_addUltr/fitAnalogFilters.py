    ...
    if options['evaluate'] == True:                            
        for analog_num in range(0, num_analog_params):
            for trial in range(0, len(train_data)):
                #new_stim.append({'gamma' : gamma[train_data[trial]], 'xi' : xi[train_data[trial]], 'num_states' : options['num_emissions'],'num_total_bins':analog_emit_w.shape[2]})
                new_stim.append({'gamma' : gamma[train_data[trial]], 'xi' : xi[train_data[trial]], 'num_states' : options['num_states'],'num_total_bins':analog_emit_w.shape[2]})
                ...
                new_stim[trial]['data']=stim[trial]
        ...
        for analog_num in range(0, num_analog_params):
            for trial in range(0, len(train_data)):
                #new_stim.append({'gamma' : gamma[train_data[trial]], 'num_states' : options['num_emissions'],'num_total_bins':analog_emit_w.shape[2]})
                new_stim.append({'gamma' : gamma[train_data[trial]], 'num_states' : options['num_states'],'num_total_bins':analog_emit_w.shape[2]})
            ...
            max_good_pts = 15000
            num_analog_iter = int(np.ceil(these_stim.shape[0] / max_good_pts))
            #print(num_analog_iter)
            #print(these_stim.shape)
            if num_analog_iter > 1:
                ...
                for nai in range(1, num_analog_iter+1):
                    iter_stim[nai-1, :] = np.floor(analog_offset * (nai - 1)) + [1, max_good_pts]
            else:
            ...
            for nai in range(0, num_analog_iter):
                use_stim = randomized_stim[int(iter_stim[nai, 0]):int(iter_stim[nai, 1])]
                
                for states in range(0, options['num_states']):
                    if options['use_ASD'] == True:
                        [out_weights, ASD_stats] = _fast_ASD_weighted_group(these_stim[use_stim, :], these_symb[use_stim], these_gamma[states, use_stim], [np.ones((int(np.round(these_stim.shape[1] / options['num_filter_bins'])), 1)) * options['num_filter_bins'], [1]], [2])
