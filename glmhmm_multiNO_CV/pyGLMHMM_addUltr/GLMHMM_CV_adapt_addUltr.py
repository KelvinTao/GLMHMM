...
class GLMHMMEstimator(BaseEstimator):
        ...
        for trial in range(0, total_trials):
            ...
            if self.analog_flag == True:                
                for analog_num in range(0, num_analog_params):
                    ...
                    #1-dim analog value
                    #num_analog_emit[analog_num] = num_analog_emit[analog_num] + np.nansum(y_analog[trial][analog_num])
    ...
    def _initialize_parameters(self, X, random_state):       
        ...
        ####################### Changed for adding ultrasound
        self.analog_emit_w_ = np.zeros((self.num_states, 1, self.num_filter_bins * self.num_feedbacks + self.filter_offset))     # states x emissions-1 x filter bins
        self.analog_emit_std_ = np.zeros((self.num_states, 1, self.num_filter_bins * self.num_feedbacks + self.filter_offset))
